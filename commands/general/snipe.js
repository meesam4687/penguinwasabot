  ...
  async execute(interaction) {
    ...
    if (!msg) {
      return interaction.reply({
        content: `Well I Did Not Find Any Deleted Message In This Channel, Lol`,
        ephemeral: true,
      });
    }
    if (msg.length > 200) {
    ...
    let embed = new Discord.EmbedBuilder()
      ...
      .setDescription(
        mesg.replace(
          /(https?:\/\/)?(www\.)?((discordapp\.com\/invite)|(discord\.gg))\/(\w+)|https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/g,
          "*link*"
        )
      )
